...
namespace lospi {
	...
	struct
		scrapifyCommand : ICommand {
		explicit scrapifyCommand(std::shared_ptr<Matterbot> bot) : bot{ bot } { }
		std::wstring get_name() override {
			return L"scraps";
		}

		std::wstring get_help() override {
			return L"`scraps`: `scraps` will call \"rivestment scraps\" to run any hashes overlooked.";
		}
		std::wstring handle_command(const std::wstring &team, const std::wstring &channel,
			...
			_sleep(2000);
			std::wstring goagain(L"rivestment scraps 240");
			return goagain;
